...
func (actor Actor) FindOrReturnPartialApp(appName string, spaceGUID string) (bool, Application, v2action.Warnings, error) {
	...
	v7App, v7AppWarnings, err := actor.V7Actor.GetApplicationByNameAndSpace(appName, spaceGUID)
	warnings = append(warnings, v7AppWarnings...)
	if err != nil {
	...
	app := Application{
		...
		Stack:       stack,
		Buildpacks:  v7App.LifecycleBuildpacks,
	}
...
func (actor Actor) updateBuildpacks(config ApplicationConfig, v2App v2action.Application) (Warnings, error) {
	...
	_, warnings, err := actor.V7Actor.UpdateApplication(v7App)
